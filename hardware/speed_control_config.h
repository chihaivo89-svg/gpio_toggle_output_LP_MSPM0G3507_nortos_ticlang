...
/*
 * 按键一键启停时的默认目标速度，单位为每 20ms 编码器脉冲数。
 * 已验证范围为 8~20。修改后 KEY1 启停将使用新速度。
 */
#define SPEED_DEFAULT_TARGET               (18)

/*
 * KEY1 从停止切换到运行前的安全等待时间。
 * 该等待由主循环按 tick_ms 非阻塞计时，不会暂停按键、OLED 或速度采样。
 */
#define SPEED_KEY1_START_DELAY_MS           (2000UL)
 ...
 * 只在左右两侧均向前运行时生效。
 */
#define SPEED_M4_TRIM_PERMILLE             (-65)
#define SPEED_M2_TRIM_PERMILLE             (65)
