...
void IMU660RB_Init(void)
{
    ...
    sampleRate = (6667 + ((0.0015 * freq_fine) * 6667)) / ODR_COEFF_208Hz;
    samplePeriod = 1.0 / sampleRate;

    /* INT1 数据就绪中断已在模块端配置好，轮询模式下不在 MCU 端使能中断 */
    // enable_group1_irq = 1;  /* 如需中断模式，取消注释并配置 INT1 GPIO 中断处理 */
...
/*
 *  仅读取 SPI 原始数据，不做姿态解算（快速，适合高频调用）
 */
void Read_IMU660RB(void)
{
    uint8_t raw[12];

    /* 合并读取：从 GYRO_OUT_L (0x22) 开始一次读 12 字节，覆盖陀螺仪+加速度计 */
    platform_read(NULL, LSM6DSR_OUTX_L_G, raw, 12);

    /* 前 6 字节 → 陀螺仪 (0x22-0x27) */
    data_raw_angular_rate[0] = (int16_t)(((uint16_t)raw[1] << 8) | raw[0]);
    data_raw_angular_rate[1] = (int16_t)(((uint16_t)raw[3] << 8) | raw[2]);
    data_raw_angular_rate[2] = (int16_t)(((uint16_t)raw[5] << 8) | raw[4]);

    /* 后 6 字节 → 加速度计 (0x28-0x2D) */
    data_raw_acceleration[0] = (int16_t)(((uint16_t)raw[7] << 8) | raw[6]);
    data_raw_acceleration[1] = (int16_t)(((uint16_t)raw[9] << 8) | raw[8]);
    data_raw_acceleration[2] = (int16_t)(((uint16_t)raw[11] << 8) | raw[10]);
...
/*
 *  基于最新读取的原始数据执行姿态解算（耗时，适合低频调用）
 */
void FusionTasks(void)
{
    FusionVector accelerometer = {acceleration_mg[0]/1000.0f, acceleration_mg[1]/1000.0f, acceleration_mg[2]/1000.0f};
    FusionVector gyroscope = {angular_rate_mdps[0]/1000.0f, angular_rate_mdps[1]/1000.0f, angular_rate_mdps[2]/1000.0f};
